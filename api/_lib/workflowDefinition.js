export const WF_AREA_ONBOARDING_DEFINITION = {
  ...
  description:
    'Runs on a 10-second timer after area/HHAH onboarding succeeds. Checks each expected HHAH for uploads; sends a notification to the HHAH login page for any missing uploads.',
  trigger: {
    id: 'trigger-time-interval',
    type: 'time_interval',
    intervalSeconds: 10,
  },
  steps: [
    {
      id: 'area-s1',
      actor: 'system',
      taskKey: 'area.uploadMonitor',
      name: 'HHAH Upload Monitor',
      description: 'Checks each expected HHAH in the statistical area for uploaded Excel + PDF ZIP. For any HHAH that has not uploaded, sends a notification to the HHAH login page. When an upload is received, Trigger 2 (HHAH Uploads Documents) fires independently and starts a new wf7 run.',
      preReq: [],
      monitorActions: [
        { key: 'check_expected_hhahs',       label: 'Check Expected HHAHs',           actor: 'system' },
        { key: 'upload_received',             label: 'Upload Received → Start wf7',    actor: 'system', condition: 'upload_received_within_24h' },
        { key: 'upload_missing_notify',       label: 'Send Notification to HHAH Login', actor: 'system', condition: 'upload_missing_after_24h' },
        { key: 'record_notification_status',  label: 'Record Notification Status',      actor: 'system', condition: 'notification_sent' },
        { key: 'wait_for_upload',             label: 'Wait for Next Check (10s)',       actor: 'system' },
      ],
    },
  ],
};
