...
| 6 | Trigger `document_upload` (legacy): this trigger type still exists in the catalog but is no longer the primary upload path. **Uploads now append to today's `daily_time` run** via `reconcileDailyRunForUpload` (see rule 8a). The `document_upload` routing + wf7 fallback have been removed from `bulk-upload/start.js`. | `reconcileDailyRunForUpload` in `api/workflows/bulk-upload/start.js`; `listActiveBuilderWorkflowsByTrigger('daily_time')` |
...
| 8a | Trigger `daily_time`: fires once per calendar day, **one shared run per day** (not one run per agency). `tickHandler` (a) creates today's run only at/after fire time when no run exists yet (run has NO base items — items are appended on demand); (b) when a run already exists (e.g. upload-created), appends ONE base item per active agency NOT yet represented (matched by `reference_payload.HHAH.id`, scanned across native base + row items), idempotent via `appendKey base:<agencyId>:<dayBucket>`. Uploads call `reconcileDailyRunForUpload` which creates/reuses the same daily run and appends `appendKey row:<hhahId>:<key>:<dayBucket>` items. Both paths share the same `sourceLabel daily:<wfId>:<dayBucket>` key so only ONE run ever exists per day. Same two callers as `time_interval` (Orchestrator poll + Vercel cron). | `tickHandler` (`dailyTimeTickHandler`); `reconcileDailyRunForUpload`; `api/_lib/dailyBucket.js` for bucket math |
...
**Agencies in the live DB (as of 2026-07-09):** Nightingale Visiting Nurses-Taunton (TEST) (`nightingale-test`/`TestAgency!2026`), Willow Creek (TEST) (contact-only demo, `willow-test`/`TestAgency!2026`), Sunrise Meadows (TEST) (manual-test-kit agency), Demo RCM Agency. Riverbend and Cedar Grove have been deleted. Each active agency gets one contact-check item per daily run.
...
- **`document_upload` trigger no longer routes uploads.** Uploads now go through `reconcileDailyRunForUpload` targeting `daily_time` workflows. The `document_upload` catalog entry still exists for legacy graph serialization but is unused in the upload routing path. Two active `daily_time` workflows would still cause duplicate appends; keep the single-active invariant.
