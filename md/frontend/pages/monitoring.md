...
## Invariants & gotchas
- **Trigger 4 runs from this page's browser tab**: the 10s `runBillingMonitor()` interval only fires while Orchestrator is open, `live` is true, and the tab is visible; `billingRunning.current` prevents overlap. Closing the tab stops the billing monitor entirely.
